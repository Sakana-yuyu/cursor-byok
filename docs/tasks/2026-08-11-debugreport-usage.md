...
2026-08-11 使用当前选中渠道重新执行该隔离探针，得到总生成 `201.8 t/s`、可见正文 `108.4 t/s`、首响应 `8.0 s`、首字 `8.5 s`。该样本仅输出聚合时序和 token 指标，未读取或记录凭据、URL、原始响应或请求标识。它证明当前模型适配器到上游的短流式链路未复现“本软件稳定压低到约 20 t/s”的现象；不能替代同一模型、同一上下文、同一推理强度下的 Cursor 客户端完整会话 E2E。网络代理和上游排队等外部条件变化时，结果可能不同。
