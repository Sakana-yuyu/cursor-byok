...
// 面板形态的供应商管理：身份由父组件（主从布局或 /supplier 薄壳路由）传入，
// 不再自己解析路由 query，从而既能嵌在 ModelConfig 右栏也能独立成页。
const props = defineProps({
  identity: { type: Object, required: true },
  autoExpandEdit: { type: Boolean, default: false },
});
...
const supplierIdentity = computed(() => props.identity || {});
const queryBaseURL = computed(() => String(supplierIdentity.value.baseURL || "").trim());
const queryGroupName = computed(() => String(supplierIdentity.value.groupName || "").trim());
...
// 编辑入口：父组件传 autoExpandEdit（对应 /supplier?edit=1 深链）时展开批量编辑面板。
// 用 watch 而非 onMounted：同一面板实例在 identity 变化时被复用，onMounted 不会再次执行。
watch(
  () => props.autoExpandEdit,
  (edit) => {
    if (edit && !bulkEditExpanded.value) {
      toggleBulkEdit();
    }
  },
  { immediate: true },
