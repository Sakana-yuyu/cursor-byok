...
function installURL(row) {
  if (row?.id === "kiro-cli") return "https://cli.kiro.dev/install";
  return "";
}
        ...
        <div v-for="row in rows" :key="row.id" class="flex min-w-0 items-center gap-3 px-3 py-2.5">
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-0.5 flex min-w-0 flex-wrap gap-x-2 text-[11px] text-[#777]">
              ...
              <a v-if="row.state === 'not_installed' && installURL(row)" :href="installURL(row)" target="_blank" rel="noreferrer" class="shrink-0 text-[#7dd3fc] hover:text-white">官方下载</a>
            </div>
          </div>
