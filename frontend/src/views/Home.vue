...
import { appState, appViewState, DEBUG_LOG_WARNING_BYTES, getCursorManualPath, openModelConfigWindow, openMetricsDetailWindow, openRequestMetricsWindow, openLocalLogsDirectory, exportLogsAction, repairProxyAction, restartCursorAction, isCursorRunningAction, repairCACorruptionAction, saveRoutingMode, saveHybridMode, syncServiceState, refreshDebugLogUsage, toUserError, toggleService } from "@/state/appState";
...
const hybridModeEnabled = computed(() => !!appState.hybridModeEnabled);
...
async function handleHybridModeChange(enabled) { if (enabled && !(await showModal({ title: "开启混合模式", content: "混合模式会在本地代理下同时启用 Cursor 官方模型：模型选择器出现官方模型列表，选官方模型时请求透传官方执行（消耗官方账号额度，官方可能风控）。确定开启吗？", confirmText: "开启混合模式", cancelText: "取消" }))) return; const result = await saveHybridMode(enabled); if (!result.ok) { await showActionError("切换失败", result.error); return; }
  // 混合模式路由在服务启动时构建，需重启 Cursor（连带服务）才生效。
  const probe = await isCursorRunningAction();
  if (probe.ok && probe.running) {
    if (await showModal({ title: "需要重启生效", content: enabled ? "已开启混合模式，重启 Cursor 后官方模型将出现在模型选择器。立即重启吗？" : "已关闭混合模式，重启 Cursor 后生效。立即重启吗？", confirmText: "立即重启", cancelText: "稍后重启" })) await handleRestartCursor({ skipConfirm: true });
    return;
  }
  message.success(enabled ? "已开启混合模式（重启 Cursor 后生效）" : "已关闭混合模式"); }
