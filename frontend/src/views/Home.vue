...
import { deleteHistoryDebugLogs, getHistorySessions } from "@/services/runtimeControlApi";
...
async function handleClearDebugLogs() {
  ...
  try {
    ...
    if (ids.length) await deleteHistoryDebugLogs(ids);
    await refreshDebugLogUsage();
    message.success("已清理调试日志");
  } catch (error) {
