...
async function handleOpenGoal() { if (isBrowserPreview) return router.push("/goal"); try { await openModelConfigWindow(); } catch (error) { await showActionError("打开失败", toUserError(error)); } }
