...
const (
	...
	// maxOutputTokensRecoveryMinBudget 是 max_output_tokens 截断恢复重试时的输出预算
	// 下限。截断（尤其思考 token 计入 max_tokens 的模型）证明原预算装不下本次输出，
	// 恢复只做一次性有界抬升到该值，不做倍数放大；目录已覆盖的思考模型在首次请求时
	// 已按目录记载的最大输出抬升预算，通常不会走到这里。
	maxOutputTokensRecoveryMinBudget = 32768
