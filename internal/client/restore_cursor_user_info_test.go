...
func TestSaveUserConfigPropagatesRestoreErrorWhenSwitchingToUpstream(t *testing.T) {
	...
	statePath := filepath.Join(cursorConfigRoot, "Cursor", "User", "globalStorage", "state.vscdb")
	if runtime.GOOS == "darwin" {
		statePath = filepath.Join(homeDir, "Library", "Application Support", "Cursor", "User", "globalStorage", "state.vscdb")
	}
	if err := os.MkdirAll(statePath, 0o755); err != nil {
		t.Fatalf("mkdir state.vscdb dir: %v", err)
	}
	// 存在有效备份 → 恢复会尝试写回官方值，打开失败即报错。
	backupPath := cursorapp.CursorAuthBackupPath()
	if err := os.MkdirAll(filepath.Dir(backupPath), 0o755); err != nil {
		t.Fatalf("mkdir backup dir: %v", err)
	}
	if err := os.WriteFile(backupPath, []byte(`{"cursorAuth/accessToken":"official-A","cursorAuth/refreshToken":"r","cursorAuth/cachedEmail":"a@real.com"}`), 0o600); err != nil {
